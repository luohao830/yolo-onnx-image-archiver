...
- OpenCodeReview workflow 默认显式写入 `llm.use_anthropic=false` 使用 OpenAI 兼容协议；设置 `OCR_LLM_REASONING_EFFORT` 且非 Anthropic 模式时，会同时写入 `thinking.type=enabled` 和 `reasoning_effort`。
